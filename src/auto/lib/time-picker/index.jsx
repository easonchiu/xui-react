...
class TimePicker extends React.PureComponent {
  ...
  initScrolls = () => {
    ...
    if (!this.iScrollDay && this.dayId && !this.props.hideDay) {
      const Key = 'Day'
  ...
  scrollEndCalc = (scroll, type) => {
    if (this['needCalc' + type]) {
      ...
      if (type === 'Day') {
        ...
        const HList = times[times.HH[hIndex]] || []
        let mIndex = HList.indexOf(this.state.MMValue)
  ...
  render() {
    ...
    const css = classnames('x-time-picker', this.props.className, {
      'x-time-picker--just-time': this.props.hideDay,
    })
    ...
    return (
      <div {...domprops} className={css}>
        ...
        {
          !this.props.hideDay && this.renderDayList()
        }
        {this.renderHHList()}
        {this.renderMMList()}
      </div>
